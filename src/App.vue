      ...
      <section v-if="page === 'copy'" class="copywriter-view">
        ...
        <div class="copy-result-card">
          <div class="copy-result-header">
            <div class="copy-result-title"><h2>生成结果</h2><span>由 {{ copyModelName || selectedTextModelName }} 生成</span></div>
            <div class="copy-result-toolbar">
              <div v-if="copyResult && copyUsageId" class="generation-feedback"><span>结果有帮助吗？</span><button :class="{ active: feedbackSelections[copyUsageId] === true }" @click="sendGenerationFeedback(copyUsageId,true)">👍 有帮助</button><button :class="{ active: feedbackSelections[copyUsageId] === false }" @click="sendGenerationFeedback(copyUsageId,false)">👎 不满意</button></div>
              <div class="copy-header-actions"><button v-if="copyResult" class="history-btn" @click="copyCopyResult">{{ copyCopied ? '✓ 已复制' : '复制文案' }}</button><button class="history-btn" @click="openHistory(true)">◷ 文案历史</button></div>
            </div>
          </div>
...
.copy-result-card { display: flex; flex-direction: column; }.copy-result-header { min-height: 42px; display: flex; align-items: center; justify-content: space-between; gap:16px; padding-bottom:12px; border-bottom:1px solid #f0f0f4 }.copy-result-title{display:flex;align-items:baseline;flex-shrink:0}.copy-result-header h2 { margin: 0 8px 0 0; font-size: 15px; }.copy-result-header .copy-result-title span { color: #858993; font-size: 10px; }.copy-result-toolbar{margin-left:auto;display:flex;align-items:center;justify-content:flex-end;gap:12px;flex-wrap:wrap}.copy-result-header .generation-feedback{padding:0;gap:6px}.copy-result-header .generation-feedback span{margin-right:2px;white-space:nowrap}.copy-result-header .generation-feedback button,.copy-header-actions .history-btn{height:32px;padding:0 11px;border-radius:9px;transition:.18s}.copy-result-header .generation-feedback button:hover,.copy-header-actions .history-btn:hover{border-color:#b9abff;background:#f5f2ff;color:#6847e8;transform:translateY(-1px)}
.copy-header-actions{display:flex;gap:7px;align-items:center;padding-left:12px;border-left:1px solid #ececf1}
...
@media(max-width:900px){.copywriter-view{grid-template-columns:1fr;height:auto}.copy-result-card{min-height:520px}}@media(max-width:700px){.copy-result-header{align-items:flex-start;flex-direction:column}.copy-result-toolbar{width:100%;margin-left:0;justify-content:space-between}.copy-header-actions{margin-left:auto}}@media(max-width:560px){.copywriter-view{padding:10px}.copy-form-card,.copy-result-card{padding:16px}.copy-grid{grid-template-columns:1fr}.copy-result-toolbar{align-items:stretch;flex-direction:column}.copy-result-header .generation-feedback{justify-content:flex-start}.copy-result-header .generation-feedback span{width:auto}.copy-header-actions{margin-left:0;padding:8px 0 0;border-left:0;border-top:1px solid #ececf1}.copy-header-actions .history-btn{flex:1}}
</style>
