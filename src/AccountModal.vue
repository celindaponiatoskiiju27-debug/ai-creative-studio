<template>
  <div class="auth-overlay" @click.self="$emit('close')">
    <section class="account-modal">
      ...
      <div class="account-actions"><button @click="$emit('works')"><span>作</span><div><b>我的作品</b><small>查看云端生成记录</small></div><i>›</i></button><button @click="$emit('recharge')"><span>充</span><div><b>获取算力</b><small>查看套餐和充值记录</small></div><i>›</i></button><button :disabled="exporting" @click="exportData"><span>备</span><div><b>{{ exporting ? '正在整理…' : '下载我的数据' }}</b><small>备份账户、作品和订单记录</small></div><i>↓</i></button></div>
...
export default {
  ...
  data: () => ({ password: '', confirmPassword: '', saving: false, exporting: false, message: '', isError: false }),
  computed: {
  ...
  methods: {
    async exportData() {
      this.exporting = true; this.message = ''; this.isError = false
      try {
        const response = await fetch('/api/me/export', { headers: { Authorization: `Bearer ${this.session.access_token}` } })
        if (!response.ok) { const data = await response.json(); throw new Error(data.error || '导出失败') }
        const blob = await response.blob(); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `lingjing-account-${new Date().toISOString().slice(0, 10)}.json`; link.click(); setTimeout(() => URL.revokeObjectURL(link.href), 1000); this.message = '个人数据备份已下载'
      } catch (error) { this.message = error.message || '导出失败，请稍后重试'; this.isError = true }
