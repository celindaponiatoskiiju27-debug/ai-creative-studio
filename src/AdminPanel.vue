...
export default {
  ...
  methods: {
    ...
    async saveAnnouncement() { this.$set(this.$data, 'announcementSaving', true); this.errorMessage = ''; try { const item = this.$data.adminAnnouncement; const body = { ...item, starts_at: item.starts_at ? new Date(item.starts_at).toISOString() : null, ends_at: item.ends_at ? new Date(item.ends_at).toISOString() : null }; const response = await fetch('/api/admin/site-announcement', { method: 'PATCH', headers: this.headers({ 'Content-Type': 'application/json' }), body: JSON.stringify(body) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || '保存全站公告失败'); await this.loadAnnouncement() } catch (error) { this.errorMessage = error.message } finally { this.$set(this.$data, 'announcementSaving', false) } },
