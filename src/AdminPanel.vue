    ...
    <section v-show="adminActiveSection === 'models'" class="model-admin-card">
      ...
      <div class="model-admin-grid">
        <article v-for="item in modelConfigs" :key="item.databaseId" :class="{ inactive: !item.enabled }">
          ...
          <label v-if="item.type === 'video'">文生视频消耗算力<input v-model.number="item.textCreditCost" type="number" min="0" max="10000" /></label>
          <div v-if="item.type === 'image'" class="model-capabilities"><label><input v-model="item.supportsGenerate" type="checkbox" /> 文生图</label><label><input v-model="item.supportsEdit" type="checkbox" /> 图片修改/合成</label></div>
